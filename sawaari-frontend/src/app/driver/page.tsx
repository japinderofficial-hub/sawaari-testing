...
export default function DriverDashboard() {
  ...
  // Live location of the driver for marker display and manual click override
  ...
  useEffect(() => {
    ...
    if (socketInstance) {
      ...
      socketInstance.on('ride_offer', (data: any) => {
  ...
  useEffect(() => {
    ...
    if (isOnline && socket && isSocketConnected) {
      const broadcastLocation = (lat: number, lng: number, heading = 0) => {
        const coords = {
          latitude: lat,
          longitude: lng,
          bearing: heading,
      ...
      if (navigator.geolocation) {
        // Immediate seed of location
        navigator.geolocation.getCurrentPosition(
          (position) => {
            console.log('📡 Initial GPS locate success, broadcasting...');
            broadcastLocation(
          ...
          (err) => {
            console.error('Initial GPS locate failed, using fallback:', err);
            // Default fallback coords to Bengaluru center
            broadcastLocation(12.9716, 77.5946);
          },
  ...
  // 13. Simulated location updates via map click
  const handleMapClick = (coords: { lat: number; lng: number }) => {
    if (activeRide) return;
    console.log('🗺️ Driver clicked map to simulate position:', coords);
    ...
    if (isOnline && socket) {
      ...
      console.log('📡 Broadcasted simulated coordinate via map click:', payload);
    }
  };
  ...
  const mapCenter = pickupCoords || driverCoords || { lat: 12.9716, lng: 77.5946 };
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col md:flex-row relative">
      ...
      {incomingOffer && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[9998] animate-fadeIn backdrop-blur-[2px]">
          <div className="bg-[#0A0A0A] border border-primary/20 rounded-2xl max-w-sm w-full p-6 space-y-6 text-center shadow-2xl z-[9999] relative">
            <div className="space-y-1">
              <span className="bg-primary/10 text-primary border border-primary/20 px-2.5 py-0.5 rounded text-[10px] font-bold tracking-wider uppercase">
                New Offer Incoming
              </span>
            ...
            <div className="border border-border bg-[#050505] p-4 rounded-xl text-left space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs pb-2 border-b border-border/50">
                <div className="space-y-0.5">
                  <p className="text-[9px] text-muted uppercase">Distance</p>
                  <p className="font-semibold text-white">
                    {incomingOffer.distanceMeters 
                      ? `${(incomingOffer.distanceMeters / 1000).toFixed(1)} km` 
                      : 'N/A'}
                  </p>
                </div>
                <div className="space-y-0.5">
                  <p className="text-[9px] text-muted uppercase">Est. Travel Time</p>
                  <p className="font-semibold text-white">
                    {incomingOffer.distanceMeters 
                      ? `${Math.max(2, Math.round((incomingOffer.distanceMeters / 1000) * 3))} mins` 
                      : 'N/A'}
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-muted font-mono">Expires in {offerTimer} seconds</p>
            </div>
            ...
            <div className="flex space-x-3 pt-2">
              <button
                ...
                className="w-1/2 py-3 bg-[#050505] border border-border text-muted hover:text-white text-xs font-semibold rounded-lg transition-colors active:scale-[0.98]"
              >
                Reject Offer
              </button>
              <button
                ...
                className="w-1/2 py-3 bg-primary hover:bg-primary-dark text-white text-xs font-semibold rounded-lg transition-colors active:scale-[0.98]"
              >
                Accept Offer
              </button>
            </div>
